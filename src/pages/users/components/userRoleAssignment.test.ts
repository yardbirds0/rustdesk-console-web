...
test('role assignment entry explicitly excludes the system owner', () => {
  expect(canShowRoleAssignmentEntry(true, false, { is_admin: true })).toBe(
    false,
  );
  expect(canShowRoleAssignmentEntry(false, true, { is_admin: true })).toBe(
    false,
  );
  expect(canShowRoleAssignmentEntry(false, true, { is_admin: false })).toBe(
    true,
  );
});
